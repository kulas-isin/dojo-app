...
export default function BattleScreen() {
  ...
  // 地盤挑戰 / 野生浪浪：對手是合成的 Fighter
  const foeSynthetic = useMemo<Pet | null>(() => {
    if (!terrH3 && !eventH3) return null;
    return {
  ...
  }, [terrH3, eventH3, foeName, foeType, foeBattle, foeLevel]);
  ...
  return (
    <Animated.View ref={rootRef} onLayout={measureAvatars} style={[styles.screen, { transform: [{ translateX: shakeA }, { scale: zoomA.interpolate({ inputRange: [0, 1], outputRange: [1, 1.06] }) }] }]}>
      ...
      {!started ? (
        <View style={styles.overlay}>
          <View style={styles.card}>
            <Text style={styles.ovTitle}>{terrH3 ? '搶地盤！🚩' : eventH3 ? '野生對戰！🐈' : '準備對戰！'}</Text>
            <Text style={styles.ovSub}>
              {terrH3 ? `${mine.name} 挑戰 ${foe.name} 鎮守的地盤`
                : eventH3 ? `一隻野生浪浪出現了！打贏有 ${rewardCans ?? 0} 🥫`
                : `${mine.name} 挑戰 ${gym?.name ?? '道館'} 主 ${foe.name}`}
            </Text>
      ...
      {result ? (
        <View style={styles.overlay}>
          <View style={styles.card}>
            <Text style={styles.ovTitle}>{result === 'win' ? (terrH3 ? '佔領成功！🚩' : eventH3 ? '打贏了！🎉' : '你贏了！👑') : '落敗…'}</Text>
            <Text style={styles.ovSub}>
              {result === 'win'
                ? terrH3
                  ? '這塊地盤插上你的旗子了，開始幫你生罐罐！'
            ...
            <Button label={terrH3 || eventH3 ? '返回地圖' : '返回道館'} onPress={() => router.back()} style={{ marginTop: spacing.md }} />
          </View>
        </View>
      ) : null}
    </Animated.View>
